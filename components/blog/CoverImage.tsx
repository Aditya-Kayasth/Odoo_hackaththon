...
const CoverImage = ({url,isEditor,setUploadedCover}:CoverImageProps) => {
  ...
  return (
    ...
    {isEditor && <div className="absolute top-8 right-5 opacity-0 group-hover:opacity-100 flex items-center gap-x-2"></div>}
    <AddCover setUploadedCover={setUploadedCover} replaceURL={url}/>
    <button className="flex items-center ml-4 gap-2" type="button" onClick={() => {handleRemoveCover(url)}}>
        <X size={20}/>
        <span >
            Remove
        </span>
